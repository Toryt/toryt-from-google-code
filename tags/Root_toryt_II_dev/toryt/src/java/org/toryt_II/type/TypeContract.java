 ...
 *            o instanceof HardInterfaceContract);
 * @invar (forall HardInterfaceContract ic; getDirectSuperInterfaceContracts().contains(ic);
 ...
 * @invar (forall HardClassContract cc; getNestedClassContracts().contains(cc);
  ...
  public class AllMembersCoveredTest extends AbstractTest {
    ...
    public final void report(PrintStream out) {
      out.println((isSuccessful() ? "success" : "FAILURE")
                  + ": "
//                  + HardTypeContract.this.getType().toString()
                  + " all members covered ("
                  + getTypeContract().getType().getName()
                  + ")");
      out.println("-------------------------------------------------------------");
      if (! isSuccessful()) {
